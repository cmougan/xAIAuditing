...
def gen_synth_shift_data(
    ...
    A = random_logit(
        ...
        * (
            gamma_A
            + gamma_shift_src * gamma_AC * C_src_vec
    ...
    T = (
        ...
        + gamma_TA * A
        + np.random.normal(
            loc=0.0, scale=scale_T + gamma_shift_src * gamma_TC * C_src_vec, size=N
        )
    )

    Y_src = (Y_src + 1) / 2
    X_src = np.stack([A, R, T], axis=1)
    data_src.append((gamma_shift_src, X_src, Y_src))
    ...
    A = random_logit(
        ...
        * (
            gamma_A
            + gamma_shift_src * gamma_AC * C_tar_vec
            + np.random.normal(loc=0.0, scale=scale_e, size=N)
        )
    )
    ...
    T = (
        ...
        + gamma_TA * A
        + np.random.normal(
            loc=0.0, scale=scale_T + gamma_shift_src * gamma_TC * C_tar_vec, size=N
        )
    )

    Y_tar = (Y_tar + 1) / 2
    X_tar = np.stack([A, R, T], axis=1)
    data_tar.append((gamma_shift_src, X_tar, Y_tar))
