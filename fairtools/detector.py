...
def shap_detector(data1, data2, verbosity=False):
    ...
    det = XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.1, verbosity=0,use_label_encoder=False)
    det.fit(X_train.values, y_train.values)

    if verbosity:
        print("AUC Train:", roc_auc_score(y_train, det.predict_proba(X_train.values)[:, 1]))
        print("AUC Test:", roc_auc_score(y_test, det.predict_proba(X_test.values)[:, 1]))
